...
class eftfactory_tricubichermite:
    ...
    def createEftWedgeXi3Zero(self, nodeScaleFactorOffset0, nodeScaleFactorOffset1):
        '''
        Create a tricubic hermite element field for a solid
        with xi3=0 collapsed, xi2 up and xi1 out .
        Each collapsed node has 3 scale factors giving the cos, sin coefficients
        of the radial line from global derivatives, plus the arc subtended by
        the element in radians, so the circumferential direction is rounded.
        Need to create a new template for each sector around axis giving common
        ...
        for layer in range(2):
            so = layer*6 + 1
            ln = layer*4 + 1
            # 2 terms for d/dxi1 via general linear map:
            remapEftNodeValueLabel(eft, [ ln ], Node.VALUE_LABEL_D_DS1, [ (Node.VALUE_LABEL_D_DS1, [so + 1]), (Node.VALUE_LABEL_D_DS3, [so + 2]) ])

            # 2 terms for cross derivative 1 2 to correct circular apex: -sin(theta).phi, cos(theta).phi
            remapEftNodeValueLabel(eft, [ ln ], Node.VALUE_LABEL_D2_DS1DS2, [ (Node.VALUE_LABEL_D_DS1, [1, so + 2, so + 3]), (Node.VALUE_LABEL_D_DS3, [so + 1, so + 3]) ])
            ...
            ln = layer*4 + 3
            # 2 terms for d/dxi3 via general linear map:
            remapEftNodeValueLabel(eft, [ ln ], Node.VALUE_LABEL_D_DS1, [ (Node.VALUE_LABEL_D_DS3, so + 4), (Node.VALUE_LABEL_D_DS1, so + 5) ])

            # 2 terms for cross derivative 1 2 to correct circular apex: -sin(theta).phi, cos(theta).phi
            remapEftNodeValueLabel(eft, [ ln ], Node.VALUE_LABEL_D2_DS1DS2, [ (Node.VALUE_LABEL_D_DS1, [1, so + 5, so + 6]), (Node.VALUE_LABEL_D_DS3, [so + 4, so + 6]) ])
        ...
        assert eft.validate(), 'eftfactory_tricubichermite.createEftWedgeXi3Zero:  Failed to validate eft'
        return eft

    def createEftWedgeXi3One(self, nodeScaleFactorOffset0, nodeScaleFactorOffset1):
        '''
        Create a tricubic hermite element field for a solid
        with xi3=1 collapsed, xi2 up and xi1 In .
        Each collapsed node has 3 scale factors giving the cos, sin coefficients
        of the radial line from global derivatives, plus the arc subtended by
        the element in radians, so the circumferential direction is rounded.
        Need to create a new template for each sector around axis giving common
        ...
        for layer in range(2):
            ...
            ln = layer * 4 + 2
            # 2 terms for d/dxi3 via general linear map:
            remapEftNodeValueLabel(eft, [ln], Node.VALUE_LABEL_D_DS1,
                                   [(Node.VALUE_LABEL_D_DS1, [so + 1]), (Node.VALUE_LABEL_D_DS3, [so + 2])])

            # 2 terms for cross derivative 1 2 to correct circular apex: -sin(theta).phi, cos(theta).phi
            remapEftNodeValueLabel(eft, [ln], Node.VALUE_LABEL_D2_DS1DS2,
                                   [(Node.VALUE_LABEL_D_DS1, [1, so + 2, so + 3]),
                                    (Node.VALUE_LABEL_D_DS3, [so + 1, so + 3])])
            ...
            # 2 terms for d/dxi3 via general linear map:
            remapEftNodeValueLabel(eft, [ln], Node.VALUE_LABEL_D_DS1,
                                   [(Node.VALUE_LABEL_D_DS3, so + 4), (Node.VALUE_LABEL_D_DS1, so + 5)])
            # 2 terms for cross derivative 1 2 to correct circular apex: -sin(theta).phi, cos(theta).phi
            remapEftNodeValueLabel(eft, [ln], Node.VALUE_LABEL_D2_DS1DS2,
                                   [(Node.VALUE_LABEL_D_DS1, [1, so + 5, so + 6]),
                                    (Node.VALUE_LABEL_D_DS3, [so + 4, so + 6])])
            # zero other cross derivative parameters
            remapEftNodeValueLabel(eft, [ln], Node.VALUE_LABEL_D2_DS1DS3, [])
            remapEftNodeValueLabel(eft, [ln], Node.VALUE_LABEL_D2_DS2DS3, [])
            remapEftNodeValueLabel(eft, [ln], Node.VALUE_LABEL_D3_DS1DS2DS3, [])
        ...
        assert eft.validate(), 'eftfactory_tricubichermite.createEftWedgeXi3One:  Failed to validate eft'
        return eft
    ...
    def createEftTetrahedronLeft(self, nodeScaleFactorOffset0, nodeScaleFactorOffset1, nodeScaleFactorOffsetUp):
        '''
        Create a tricubic hermite element field for a solid tetrahedron for the axis of a
        solid sphere pole, with xi1 and xi3 collapsed on xi2 = 1, and xi3 collapsed on xi1 = 0.
        Each collapsed node has 3 scale factors giving the cos, sin coefficients of
        the radial line from global derivatives, plus the arc subtended by the element in radians,
        so the circumferential direction is rounded. Collapsed nodes on xi2 = 1 have 2 additional
        scale factors cos and sin coefficients of the inclination angle.
        ...
        # remap parameters on xi3 = 0 before collapsing nodes
        remapEftNodeValueLabel(eft, [ 1, 5 ], Node.VALUE_LABEL_D_DS3, [])

        soAround = 1
        remapEftNodeValueLabel(eft, [ 1 ], Node.VALUE_LABEL_D_DS1,
                               [ (Node.VALUE_LABEL_D_DS3, [soAround + 1]), (Node.VALUE_LABEL_D_DS1, [soAround + 2]) ])
        # remapEftNodeValueLabel(eft, [ 1 ], Node.VALUE_LABEL_D2_DS1DS3, [ (Node.VALUE_LABEL_D_DS1, [1, soAround + 2, soAround + 3]), (Node.VALUE_LABEL_D_DS3, [soAround + 1, soAround + 3]) ])
        remapEftNodeValueLabel(eft, [ 5 ], Node.VALUE_LABEL_D_DS1,
                               [(Node.VALUE_LABEL_D_DS3, [soAround + 4]), (Node.VALUE_LABEL_D_DS1, [soAround + 5])])
        # remapEftNodeValueLabel(eft, [ 2 ], Node.VALUE_LABEL_D2_DS1DS3, [ (Node.VALUE_LABEL_D_DS1, [1, soAround + 5, soAround + 6]), (Node.VALUE_LABEL_D_DS3, [soAround + 4, soAround + 6]) ])

        ln_map = [ 1, 2, 3, 3, 1, 4, 3, 3 ]
        remapEftLocalNodes(eft, 4, ln_map)

        assert eft.validate(), 'eftfactory_tricubichermite.createEftTetrahedronLeft:  Failed to validate eft'
        return eft


    def createEftTetrahedronRight(self, nodeScaleFactorOffset0, nodeScaleFactorOffset1, nodeScaleFactorOffsetUp):
        '''
        Create a tricubic hermite element field for a solid tetrahedron for the axis of a
        solid sphere pole, with xi1 and xi3 collapsed on xi2 = 1, and xi3 collapsed on xi1 = 1.
        Each collapsed node has 3 scale factors giving the cos, sin coefficients of
        the radial line from global derivatives, plus the arc subtended by the element in radians,
        so the circumferential direction is rounded. Collapsed nodes on xi2 = 1 have 2 additional
        scale factors cos and sin coefficients of the inclination angle.
        ...
        # remap parameters on xi3 = 0 before collapsing nodes
        remapEftNodeValueLabel(eft, [ 2, 6 ], Node.VALUE_LABEL_D_DS3, [])

        soAround = 1
        remapEftNodeValueLabel(eft, [ 2 ], Node.VALUE_LABEL_D_DS1,
                               [ (Node.VALUE_LABEL_D_DS3, [soAround + 1]), (Node.VALUE_LABEL_D_DS1, [soAround + 2]) ])
        # remapEftNodeValueLabel(eft, [ 1 ], Node.VALUE_LABEL_D2_DS1DS3, [ (Node.VALUE_LABEL_D_DS1, [1, soAround + 2, soAround + 3]), (Node.VALUE_LABEL_D_DS3, [soAround + 1, soAround + 3]) ])
        remapEftNodeValueLabel(eft, [ 6 ], Node.VALUE_LABEL_D_DS1,
                               [(Node.VALUE_LABEL_D_DS3, [soAround + 4]), (Node.VALUE_LABEL_D_DS1, [soAround + 5])])
        # remapEftNodeValueLabel(eft, [ 2 ], Node.VALUE_LABEL_D2_DS1DS3, [ (Node.VALUE_LABEL_D_DS1, [1, soAround + 5, soAround + 6]), (Node.VALUE_LABEL_D_DS3, [soAround + 4, soAround + 6]) ])

        ln_map = [ 1, 2, 3, 3, 4, 2, 3, 3 ]
        remapEftLocalNodes(eft, 4, ln_map)

        assert eft.validate(), 'eftfactory_tricubichermite.createEftTetrahedronRight:  Failed to validate eft'
        return eft
    ...
    # def createEftTetrahedronXi2One(self, nodeScaleFactorOffset0, nodeScaleFactorOffset1):
    #     '''
    #     Create a tricubic hermite element field for a solid tetrahedron for the apex of cecum,
    #     ========with xi1 and xi3 collapsed on xi2 = 0, and xi3 collapsed on xi1 = 1 and xi2 = 1.
    #     with xi2 and xi3 collapsed on xi1 = 0, and xi3 collapsed on xi1 = 1 and xi2 = 1.
    #     ======Each collapsed node on xi2 = 0 has 3 scale factors giving the cos, sin coefficients of
    #     Each collapsed node on xi1 = 0 has 3 scale factors giving the cos, sin coefficients of
    #     the radial line from global derivatives, plus the arc subtended by the element in radians,
    #     so the circumferential direction is rounded.
    #     Need to create a new template for each sector around axis giving common
    #     nodeScaleFactorOffset values on common faces. Suggestion is to start at 0 and
    ...
    #
    #     # remap parameters on xi2 = 0 before collapsing nodes
    #     remapEftNodeValueLabel(eft, [ 1, 3, 5, 7], Node.VALUE_LABEL_D_DS2, [])
    #     remapEftNodeValueLabel(eft, [ 1, 3, 5, 7], Node.VALUE_LABEL_D_DS3, [])
    #
    #     for layer in range(2):
    #         soAround = 1
    ...
    #         remapEftNodeValueLabel(eft, [ ln ], Node.VALUE_LABEL_D3_DS1DS2DS3, [])
    #
    #     remapEftNodeValueLabel(eft, [ 1, 3, 5, 7 ], Node.VALUE_LABEL_D_DS3, [ (Node.VALUE_LABEL_D_DS1), [1]])
    #
    #     # remap parameters on xi1 = 0, xi2 = 1 before collapsing nodes
    #     remapEftNodeValueLabel(eft, [3, 7], Node.VALUE_LABEL_D_DS3, [])
    #     remapEftNodeValueLabel(eft, [3, 7], Node.VALUE_LABEL_D2_DS1DS3, [])
    ...
    #     remapEftNodeValueLabel(eft, [3, 7], Node.VALUE_LABEL_D3_DS1DS2DS3, [])
    #
    #     ln_map = [ 1, 2, 1, 3, 1, 4, 1, 3]
    #     remapEftLocalNodes(eft, 4, ln_map)
    #
    #     assert eft.validate(), 'eftfactory_tricubichermite.createEftTetrahedronXi2One:  Failed to validate eft'
    #     return eft
    #
    #
    # def createEftTetrahedronXi2Zero(self, nodeScaleFactorOffset0, nodeScaleFactorOffset1):
    #     '''
    #     Create a tricubic hermite element field for a solid tetrahedron for the apex of cecum,
    #     ==========with xi1 and xi3 collapsed on xi2 = 0, and xi3 collapsed on xi1 = 0, xi2 = 1.
    #     with xi2 and xi3 collapsed on xi1 = 1, and xi3 collapsed on xi1 = 0, xi2 = 1.
    #     =======Each collapsed node on xi2 = 0 has 3 scale factors giving the cos, sin coefficients of
    #     Each collapsed node on xi1 = 1 has 3 scale factors giving the cos, sin coefficients of
    #     the radial line from global derivatives, plus the arc subtended by the element in radians,
    #     so the circumferential direction is rounded.
    #     Need to create a new template for each sector around axis giving common
    #     nodeScaleFactorOffset values on common faces. Suggestion is to start at 0 and
    ...
    #     remapEftNodeValueLabel(eft, [ 2, 4, 6, 8 ], Node.VALUE_LABEL_D_DS2, [])
    #     remapEftNodeValueLabel(eft, [ 2, 4, 6, 8 ], Node.VALUE_LABEL_D_DS3, [])
    #
    #     for layer in range(2):
    #         soAround = 1
    ...
    #         remapEftNodeValueLabel(eft, [ ln ], Node.VALUE_LABEL_D3_DS1DS2DS3, [])
    #
    #     remapEftNodeValueLabel(eft, [ 2, 4, 6, 8 ], Node.VALUE_LABEL_D_DS3, [ (Node.VALUE_LABEL_D_DS1), [1]])
    #
    #     # remap parameters on xi1 = 0, xi2 = 1 before collapsing nodes
    #     remapEftNodeValueLabel(eft, [4, 8], Node.VALUE_LABEL_D_DS3, [])
    #     remapEftNodeValueLabel(eft, [4, 8], Node.VALUE_LABEL_D2_DS1DS3, [])
    ...
    #     remapEftNodeValueLabel(eft, [4, 8], Node.VALUE_LABEL_D3_DS1DS2DS3, [])
    #
    #     ln_map = [ 1, 2, 3, 1, 4, 1, 3, 1]
    #     remapEftLocalNodes(eft, 4, ln_map)
    #
    #     assert eft.validate(), 'eftfactory_tricubichermite.createEftTetrahedronXi2Zero:  Failed to validate eft'
    #     return eft
