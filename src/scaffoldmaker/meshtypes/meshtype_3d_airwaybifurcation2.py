...
class MeshType_3d_airwaybifurcation2(Scaffold_base):
    ...
    def checkOptions(options):
        ...
        if (options['Daughter1 angle'] < 5):
            options['Daughter1 angle'] = 5
        if (options['Daughter1 angle'] > 60):
            options['Daughter1 angle'] = 60

        if (options['Daughter2 angle'] < 5):
            options['Daughter2 angle'] = 5
    ...
    def generateBaseMesh(region, options):
        ...
        parentsegmentLength = [2.38, 2.74, 1.71] ##When using centerline this is trach length - junctionheight
        daughter1segmentLength = [2.7, 1.27, 1.0]  ##When using centerline this is trach length - junctionheight
        daughter2segmentLength = [1.74, 1.2, 1.1] ##When using centerline this is trach length - junctionheight

        startRadiusparentlist = [0.72, 0.52, 0.75]
        endRadiusparentlist = startRadiusparentlist
        startRadiusDaugh1list = [0.52, 0.35, 0.6]
        ...
        dirvecparentlist = [[-0.01385,-0.04533,0.99888],[0.28833,-0.01513,0.95741],[-0.20846,-0.07799,0.97492]]
        ...
        dirvecdaughter1list = [[0.28833,-0.01513,0.95741],[0.4100,1.7100,0.93000],[-0.1900,-0.34000,2.52000]]
        dirvecdaughter2list= [[-0.20846,-0.07799,0.97492],[-0.150,-1.940,1.55000],[-0.290,0.3700,2.180]]

        radiusvecparentlist =  [[0.0, 1.0, 0.0],[-0.08206,5.1855,1.55829],[-0.27244,-3.39860,0.73252]]

        radiusvecdaughter1list = [[-0.95758,-0.01515,0.28776]]
        radiusvecdaughter2list = [[0.97456,-0.07812,0.21005]]

        parentx0list = [[0,0,0],[-0.1080,-0.0330,2.380],[-0.108,-0.0330,2.380]]
        daughter1x0list = [[-0.1080,-0.0330,2.38000],[-0.190,1.5300,7.57000],[-0.3800,-0.760,5.780]]
        daughter2x0list = daughter1x0list

        daughter1x1list = [[-0.190,1.530,7.570],[0.220,3.2400,8.500],[-0.570,-1.100,8.300]]
        daughter2x1list = [[-0.3800,-0.7600,5.78000],[-0.530,-2.70000,7.33000],[-0.48000,1.90000,9.75000]]

        segmentlengthparentlist = [2.38, 2.71, 1.74]
        segmentlengthdaughter1list = [2.71, 1.0, 1.275]
        segmentlengthdaughter2list = [1.74, 1.11, 1.24]

        #####################################################################

        #SMOOTHING PARAMETERS FOR EVERY SEGMENT
        xlensegmentparentlist = [0.8, 0.35, 0.45]
        xlensegmentd1list = [0.3, 0.3, 0.3]
        xlensegmentd2list = [0.3, 0.34, 0.34]
        ...
        for nSegment in range(segmentCount):
            ...
            cd2daughter1 = [[-0.95758,-0.01515,0.28776], [-0.95758,-0.01515,0.28776]]
            ...
            cd2daughter2 = [[0.97456,-0.07812,0.21005], [0.97456,-0.07812,0.21005]]
